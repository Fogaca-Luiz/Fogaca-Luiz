"""
Atualiza a planilha BB.xlsx com os lancamentos reais encontrados nos
extratos .csv salvos na subpasta "BB".

Uso:
    python atualizar_extrato_BB.py

Regras aplicadas:
  - So sao considerados lancamentos reais (linhas "Saldo Anterior" e
    "S A L D O" sao ignoradas, pois sao marcadores de saldo).
  ...
  - O periodo coberto pelos extratos e definido pela primeira e ultima
    data encontradas nos arquivos .csv da pasta BB. Dentro desse
    periodo, a planilha e comparada DIA A DIA com o extrato:
      - Se os valores de um dia na planilha ja batem exatamente com os
        ...
        Anterior" do BB, repitam 1-2 dias que ja estavam corretos).
...
XLSX_PATH = SCRIPT_DIR / "BB.xlsx"
CSV_DIR = SCRIPT_DIR / "BB"
SHEET_NAME = "BB"
...
SALDO_MARKERS = ("SALDO ANTERIOR", "S A L D O")
...
def extrair_lancamentos(caminho_csv):
    """Le um extrato .csv do BB e retorna uma lista de (data, texto, valor).

    Colunas do csv: Data, Dependencia Origem, Historico, Data do Balancete,
    Numero do documento, Valor.
    """
    df = pd.read_csv(caminho_csv, header=None, encoding="cp1252", quotechar='"')
    registros = []
    ...
    for _, linha in df.iterrows():
        ...
        if data is None:
            if dentro_da_tabela:
                # Fim do bloco de lancamentos reais.
                break
            continue
...
def main():
    ...
    if not CSV_DIR.exists():
        raise FileNotFoundError(f"Pasta de extratos nao encontrada: {CSV_DIR}")
